...
public class HostService {
	...
	public Map<String, Object> Login(ConsumerRequest cr) {
		Map<String, Object> responseData = Config.leStartSession(cr.getEmail());
//		defaultPromptChallenge((String) responseData.get("sessionToken"));
		return responseData;
	}

	public Map<String, Object> pollStatus(AgentConfigRequest acr) throws Exception {
		return Challenges.pollStatus(acr.getSessionToken());
	}

	public Map<String, Object> promptChallenge(PromptRequest pr) {
		return Challenges.addPromptChallenge(pr.getSessionToken(), pr.getQues(), pr.getAns(),
				pr.getRequired());
	}
	
	public Map<String, Object> hostBehaviorChallenge(HostBehaviorRequest br) {
		return Challenges.addHostBehaviorChallenge(br.getSessionToken(), br.getOrientation(), br.getTouches(),
				br.getRequired());
	}

	public Map<String, Object> hostBehaviorV6Challenge(HostBehaviorRequest br) {
		return Challenges.addHostBehaviorV6Challenge(br.getSessionToken(), br.getTouches(),
				br.getRequired());
	}
	
	public Map<String, Object> latLongChallenge(LocationRequest lr) {
		return Challenges.addLatLongChallenge(lr.getSessionToken(), lr.getLatitude(),
				lr.getLongitude(), lr.getRadius(), lr.getRequired());
	}

	public Map<String, Object> latLongV6Challenge(LocationRequest lr) {
		return Challenges.addLatLongV6Challenge(lr.getSessionToken(), lr.getLatitude(),
				lr.getLongitude(), lr.getRadius(), lr.getInout(), lr.getRequired());
	}

	public Map<String, Object> biometricChallenge(BiometricRequest br) {
		return Challenges.addBiometricChallenge(br.getSessionToken(), br.getTouches(),
				br.getRequired());
	}

	public Map<String, Object> timeChallenge(TimeRequest tr) {
		return Challenges.addTimeChallenge(tr.getSessionToken(), tr.getStartDate(), tr.getEndDate(),
				tr.getInout(), tr.getRequired());
	}

	public Map<String, Object> wearableChallenge(WearableRequest wr) {
		return Challenges.addWearableChallenge(wr.getSessionToken(), wr.getDeviceId(),
				wr.getRequired());
	}
	...
	public Map<String, Object> device(DeviceRequest dr) throws Exception {
		...
		if ("HOST".equalsIgnoreCase(dr.getChallengeType())) {
			...
			if (dr.getChallengeDetails().containsKey("LAT_LONG_V6")) {
				...
				Map<String, Object> map = (HashMap<String, Object>) dr.getChallengeDetails().get("LAT_LONG_V6");
			...
			if (dr.getChallengeDetails().containsKey("HOST_BEHAVIOR_V6")) {
				...
				Map<String, Object> map = (HashMap<String, Object>) dr.getChallengeDetails().get("HOST_BEHAVIOR_V6");
//				br.setOrientation("0");
...
//	private void defaultPromptChallenge(String sessionToken, String agentId) {
//		PromptRequest pr = new PromptRequest();
//		pr.setSessionToken(sessionToken);
//		pr.setQues("PIN DEMO");
//		pr.setAns("2019");
//		pr.setRequired("true");
//		promptChallenge(pr);
